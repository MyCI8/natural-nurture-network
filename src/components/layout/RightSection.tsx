...
const RightSection = () => {
  ...
  const getVideoLinks = (): VideoLink[] => {
    ...
    if (Array.isArray(articleData.video_links)) {
      return articleData.video_links.filter(link => link && typeof link.url === 'string' && link.url.trim() !== '');
    }
    if (typeof articleData.video_links === 'string') {
      try {
        const parsed = JSON.parse(articleData.video_links);
        if (Array.isArray(parsed)) {
          return parsed.filter(link => link && typeof link === 'object' && typeof link.url === 'string' && link.url.trim() !== '' && typeof link.title === 'string');
        }
      } catch (e) {
  ...
  return <div className="h-full flex flex-col relative">
      <Separator orientation="vertical" className="absolute left-0 top-0 bottom-0 w-[1px] bg-border dark:bg-gray-700 z-10" />
      ...
      <div className="p-4 h-full flex flex-col overflow-hidden">
        {newsArticleId && videoLinks.length > 0 ? <>
          ...
          </> : newsArticleId && articleLoading ? <div className="flex items-center justify-center p-8">
            <div className="animate-spin rounded-full h-8 w-8 border-t-2 border-b-2 border-primary"></div>
          </div> : newsArticleId && !videoLinks.length ? <div className="p-4 text-center">
            <p className="text-muted-foreground dark:text-dm-text-supporting">No videos available for this article</p>
          </div> : null}
        
        {(location.pathname === '/news' || location.pathname === '/news/') && <div className="h-full flex flex-col">
            <h2 className="text-xl sm:text-2xl font-bold mb-3 sm:mb-4 text-left pl-2">Latest Videos</h2>
            {videos && videos.length > 0 ? <div className="flex-1 flex flex-col space-y-6">
                {videos.map((video, index) => <div key={video.id} className="flex flex-col">
                    <div onClick={() => handleVideoClick(video)} className="cursor-pointer touch-manipulation">
              ...
              </div> : <p className="text-muted-foreground dark:text-dm-text-supporting text-left pl-2">No videos available</p>}
          </div>}
                ...
                <div className="p-3 border-b border-gray-200 dark:border-gray-800 px-0 py-0">
                  ...
                  <div className="flex flex-col space-y-2">
                    ...
                    <div className="text-sm text-gray-500 mb-2 flex items-center gap-2">
                      ...
                      <Button variant="ghost" size="icon" className={`p-0 h-6 w-6 hover:bg-transparent ${userLikeStatus ? 'text-red-500' : 'text-gray-500'}`} onClick={handleLike}>
