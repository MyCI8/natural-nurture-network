...
const ExploreVideos = () => {
  ...
  const [videoType, setVideoType] = useState<"all" | "general" | "news" | "explore">("explore");
  const [deleteVideoId, setDeleteVideoId] = useState<string | null>(null);
  const [isDeleteDialogOpen, setIsDeleteDialogOpen] = useState(false);

  const { data: videos = [], isLoading, refetch } = useQuery({
    queryKey: ["admin-explore-videos", searchQuery, currentTab, sortBy, videoType],
    queryFn: async () => {
      let query = supabase
        .from("videos")
      ...
      if (videoType !== "all") {
        ...
        query = query.eq('video_type', 'explore');
      }
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4 mb-6">
        ...
        <Card>
          <CardContent className="p-4">
            ...
            <p className="text-xs text-muted-foreground mt-1">
              {Math.round((videosWithLinks / (totalVideos || 1)) * 100)}% of videos
            </p>
          </CardContent>
        </Card>
        ...
        <Card>
          <CardContent className="p-4">
            <div className="flex justify-between items-start">
              <div>
                <p className="text-sm font-medium text-muted-foreground">Explore Videos</p>
                <p className="text-2xl font-bold mt-1">
                  {videos.filter(v => v.video_type === 'explore').length}
                </p>
              </div>
              <VideoIcon className="h-5 w-5 text-muted-foreground" />
            </div>
            <p className="text-xs text-muted-foreground mt-1">Public feed videos</p>
          </CardContent>
        </Card>
      </div>
    );
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div className="flex items-center gap-2">
          ...
          <div>
            <h2 className="text-2xl font-bold">Manage Explore Videos</h2>
            <p className="text-muted-foreground">
              Create and manage videos for explore feed
            </p>
          </div>
        </div>
        <Button onClick={handleAddVideo}>
          <Plus className="mr-2 h-4 w-4" /> Add Explore Video
        </Button>
      </div>
      ...
      <Card>
        <CardHeader className="pb-3">
          <CardTitle className="text-lg">Explore Video Library</CardTitle>
        </CardHeader>
        <CardContent>
          <Tabs 
            defaultValue="all" 
          ...
          >
            ...
            <TabsContent value="draft" className="mt-6">
              <VideoGrid 
                videos={videos.filter(v => v.status === 'draft')} 
                navigate={navigate}
                onDeleteClick={handleDeleteClick}
              />
            </TabsContent>
